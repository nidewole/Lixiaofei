...
import com.okgo.interceptor.LoggerInterceptor;
 ...
 */
public class OkGo {
    public static final int DEFAULT_MILLISECONDS = 10000;       //默认的超时时间 10s

    private static OkGo mGo;                                //单例
    private Handler mDeliveryHandler;                                  //用于在主线程执行的调度器
    ...
    private OkGo() {
        ...
        okHttpClientBuilder.connectTimeout(DEFAULT_MILLISECONDS, TimeUnit.MILLISECONDS);//设置连接超时
        okHttpClientBuilder.readTimeout(DEFAULT_MILLISECONDS, TimeUnit.MILLISECONDS);//设置读取超时
        okHttpClientBuilder.writeTimeout(DEFAULT_MILLISECONDS, TimeUnit.MILLISECONDS);//设置写入超时
